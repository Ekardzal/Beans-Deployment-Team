...
if exist "%cd%\venv" (
    ...
    for /f "tokens=1,* delims=: " %%A in (config.yaml) do (
        if "%%A"=="users" set USERS=%%B
        if "%%A"=="spawn_rate" set SPAWN_RATE=%%B
        if "%%A"=="run_time" set RUN_TIME=%%B
    )
    
    if "!USERS!"=="" (
        echo Missing 'users' in config.yaml
        goto :ERROR
    )
    if "!SPAWN_RATE!"=="" (
        echo Missing 'spawn_rate' in config.yaml
        goto :ERROR
    )
    if "!RUN_TIME!"=="" (
        echo Missing 'run_time' in config.yaml
        goto :ERROR
    )
    
    echo Starting test with !USERS! users, !SPAWN_RATE! spawn rate, !RUN_TIME! runtime
    locust -f EndpointLimitTest.py --headless -u !USERS! -r !SPAWN_RATE! --run-time !RUN_TIME! --csv=results
	start http://localhost:8089 || (echo Error bei: "start http://localhost:8089")
    if errorlevel 1 (
        echo Load test failed with errorlevel !errorlevel!
        goto :ERROR
    )
