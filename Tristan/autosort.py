...
model = YOLO(r"F:\Unii\AAAAAAA\best.pt")                                                                           # Model laden

source = r"F:\Unii\test\imgstore"                                                                            # Quellordner, der die Bilder fuer die Prediction enthaelt

results = model(source, conf=0.59)                                                                         # Prediction-Befehl mit Confidence-Threshold von 0.59

sorted_results_dir = r"F:\Unii\test\csv"                                                                     # Output-Ordner der .csv-Datei
sorted_images_dir = r"F:\Unii\test\sorted"                                                                # Output-Ordner der sortierten Bilder
...
for result in results: 
    ...
    for box in boxes:
        ...
        detections.append((file_name,str(confidence).replace(".", ","), class_name, x1,y1,x2,y2))                                  # fuegt extrahierte Details in eine Liste zusammen
